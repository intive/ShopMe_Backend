...
public class User {
    ...
    @NotEmpty
    @Size(min = PASSWORD_MIN_LENGTH, message = "Password contains not enough characters, minimum is " + PASSWORD_MIN_LENGTH + "." )
    @Size(max = PASSWORD_MAX_LENGTH, message = "Password contains too much characters, maximum is " + PASSWORD_MAX_LENGTH + "." )
    @Pattern(regexp = AT_LEAST_ONE_DIGIT_OCCURS, message = "Password need contain at least one digit")
    @Pattern(regexp = AT_LEAST_ONE_UPPER_CASE_LETTER_OCCURS, message = "Password need contain at least one upper case letter")
    @ApiModelProperty(value = "Represents user's password", required = true, position = 5, example = "Password9")
    private String password;

    @NotEmpty
    @Size(min = PHONE_NUMBER_MIN_LENGTH, message = "Phone number contains not enough characters, minimum is " + PHONE_NUMBER_MIN_LENGTH + "." )
    @Size(max = PHONE_NUMBER_MAX_LENGTH, message = "Phone number contains too much characters, maximum is " + PHONE_NUMBER_MAX_LENGTH + "." )
    @Pattern(regexp = ACCEPTABLE_ONLY_DIGITS, message = "The user's phone number should contain only digits.")
    @ApiModelProperty(value = "Represents user's phone number", required = true, position = 6, example = "0234567890")
    private String phoneNumber;

    @NotEmpty
    @Size(min = BANK_ACCOUNT_LENGTH, max = BANK_ACCOUNT_LENGTH, message = "Bank account number should contain " + BANK_ACCOUNT_LENGTH + " digits.")
